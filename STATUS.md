# Dashboard Studio — STATUS (hourly-loop resume anchor)
...
- **License.** The Studio is proprietary — see `LICENSE` (© 2026 Polecat.live; all rights reserved).
  ...
  files may carry a one-line header (`/* Dashboard Studio — © 2026 Polecat.live. See LICENSE. */`).
...
- v26: **auto-publish CI** (historical — a separate-repo mirror workflow, superseded by the v60
  migration; this repo now auto-deploys from `main` via GitHub Pages). Plus **a11y keyboard focus
  rings** (`:focus-visible` outline themed via --pentaho; covered by a Tab test). 88/88.
...
layouts and interactions. Periodically (≈ when
