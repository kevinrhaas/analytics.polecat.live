...
> **Z8 follow-ups (not yet done, the bulk of the track):** the Table-specific extras called out above (row
> limit/paging/sort/subtotals/freeze-header/row-density — table already has some via v113's interactive
> table, worth auditing against this list), a real per-type capability map covering ALL ~51 types' *own*
> option sets (not just the 5 interaction sections), and the inline visual setting hints (tiny before/after
