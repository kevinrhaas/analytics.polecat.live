...
(function () {
  ...
  // R5+ slice 2: promoted to Studio.lsGet/Studio.lsSet (model.js) so app/branding-defaults.js
  // can share the same implementation; these are just the local aliases the rest of this file
  // already calls by their short names.
  var lsGet = Studio.lsGet, lsSet = Studio.lsSet;
  ...
  // R5+ slice 2 (tech-debt sweep): the default-field/preset "config layer" (dashboard
  // defaults + style/template-var/custom-theme presets) moved to app/branding-defaults.js —
  // these are just the local aliases the rest of this file already calls by their short
  // names. defaultDashboardTheme's one bit of studio.js-only state (falling back to the
  // live app Color theme, S.appTheme) is wired in once via configureBrandingDefaults, right
  // after appThemeToDashboardTheme/appTheme are defined below.
  var strDefault = Studio.strDefault, setStrDefault = Studio.setStrDefault;
  var defaultSubtitle = Studio.defaultSubtitle, setDefaultSubtitle = Studio.setDefaultSubtitle;
  var defaultAccentColor = Studio.defaultAccentColor, setDefaultAccentColor = Studio.setDefaultAccentColor;
  var defaultHeaderBg = Studio.defaultHeaderBg, setDefaultHeaderBg = Studio.setDefaultHeaderBg;
  var defaultTitleSize = Studio.defaultTitleSize, setDefaultTitleSize = Studio.setDefaultTitleSize;
  var defaultSubtitleStyle = Studio.defaultSubtitleStyle, setDefaultSubtitleStyle = Studio.setDefaultSubtitleStyle;
  var defaultDashboardTheme = Studio.defaultDashboardTheme, setDefaultDashboardTheme = Studio.setDefaultDashboardTheme;
  var defaultCardSkin = Studio.defaultCardSkin, setDefaultCardSkin = Studio.setDefaultCardSkin;
  var defaultLogo = Studio.defaultLogo, setDefaultLogo = Studio.setDefaultLogo;
  var stylePresets = Studio.stylePresets, saveStylePresetList = Studio.saveStylePresetList,
    addStylePreset = Studio.addStylePreset, deleteStylePreset = Studio.deleteStylePreset, applyStylePreset = Studio.applyStylePreset;
  window.__studioStylePresets = stylePresets; // test hook
  var templateVarSets = Studio.templateVarSets, saveTemplateVarSetList = Studio.saveTemplateVarSetList,
    addTemplateVarSet = Studio.addTemplateVarSet, deleteTemplateVarSet = Studio.deleteTemplateVarSet, applyTemplateVarSet = Studio.applyTemplateVarSet;
  window.__studioTemplateVarSets = templateVarSets; // test hook
  var customThemePresets = Studio.customThemePresets, saveCustomThemePresetList = Studio.saveCustomThemePresetList,
    addCustomThemePreset = Studio.addCustomThemePreset, deleteCustomThemePreset = Studio.deleteCustomThemePreset, applyCustomThemePreset = Studio.applyCustomThemePreset;
  window.__studioCustomThemePresets = customThemePresets; // test hook
  var applyDashboardDefaults = Studio.applyDashboardDefaults;
  ...
  // R5+ slice 2: the one bit of branding-defaults.js's config layer that needs live studio.js
  // state (defaultDashboardTheme()'s "never-set" fallback follows the app's own Color theme) —
  // injected once so the extracted module needs no other dependency.
  Studio.configureBrandingDefaults(function () { return appThemeToDashboardTheme(appTheme()); });
