# Service-worker release notes — the archive (v19 → v488)

Every time `sw.js` bumped its `CACHE_NAME`, the bump carried a comment saying what
changed and which precached files rolled with it. Those notes are all here, newest
first. They were moved out of `sw.js` itself by **AUD-08** (see `AUDIT-2026-08.md`
§1.4 and STATUS.md): 2,454 lines of history sat ahead of the worker code in a file
the browser re-fetches on every update check, so the service worker weighed ~190KB
to ship ~5KB of logic. Nothing read the notes at runtime — they are documentation,
and documentation belongs here.

**Going forward there is nothing to append to this file.** `sw.js` keeps only its
`CACHE_NAME`; what shipped in a release lives in `js/changelog.js` (the fleet
changelog, which is also what users read), and which precached files rolled is
visible in the commit that bumped the cache. This archive is the record of the
older convention, kept because those notes carry real forensic detail about how
the precache list evolved.

---
## v488

AUD-11 — the copy/doc truth pass: one chart-type number (54) across marketing and Help, the Simple-mode count corrected to 15, GPL-3.0 license copy, a truthful "What's next" list, and tools/doc-truth.mjs in the dev gate to keep them honest; precached index.html, app/studio.js, docs/index.html, js/changelog-head.js changed.

## v486

AUD-12 — ⌘K and the Admin page's Section access card are both built from the rail itself (app/shell.js __studioRailSections), so ⌘K reaches all 13 sections (and only the ones your account may open) and every hideable section is offered; precached js/changelog.js, app/shell.js, app/palette.js, app/studio.js, docs/index.html changed.

## v485

AUD-06 slice 4 — dates compare as dates in the shared filter predicate and in DA sort rules (day granularity for a plain date, UTC for a zoneless value); the job engine's standby copy of the predicate is gone; precached js/changelog.js, app/model.js, app/studio.js, app/jobs.js, app/sources/jobs-engine.js, docs/index.html changed.

## v484

AUD-09 dead code — app/gate-config.js (a retired passcode gate's two live access-code hashes, zero readers) DELETED and dropped from this precache list; app/index.html, app/viewer.html changed; vendor/dashkit.js lost the uncalled TIME_RANGE/fromkey pair.

## v482

AUD-06 slice 3 — one filter-operator vocabulary (Studio.filterOps) behind both the DA output rules and the job Filter step; precached js/changelog.js, app/model.js, app/jobs.js, app/sources/jobs-engine.js, docs/index.html changed.

## v481

AUD-06 — one shared catalog view-mode kit (Studio.catalogView), one default (list) for all six panels; precached js/changelog.js, app/studio.js, app/views.js, app/datasets.js, app/connections.js, app/jobs.js, docs/index.html changed.

## v477

AUD-05 — the builder/preview/Help postMessage channel is origin+source checked; precached js/changelog.js, app/studio.js, app/studio-render.js, docs/index.html changed.

## v476

SWEEP574-3b (table family) — clickable table rows are keyboard-operable in exports; precached js/changelog.js, app/studio-charts.js, docs/index.html changed.

## v475

SWEEP574-3b — bar/donut/treemap marks are keyboard-operable in exports; precached js/changelog.js, app/studio-charts.js, docs/index.html changed.

## v471

DEMO-LOCAL-2 — admin/admin always opens the local workspace; precached app/gate.js, js/changelog.js changed.

## v470

AUD-03 — session-only vault passphrase + salted/iterated password digests; precached js/changelog.js, app/auth.js, app/sources/sync.js, app/studio.js, app/connections.js changed.

## v467

pipeline adoption — the precached
js/changelog.js gained the v835 staged-releases entry.

## v466

AUD-04 data-loss guards — the
...
TOUR-FRONT + ADOPT-COPY — welcome hero
...
KEVIN-LIVE-2 — same-url re-binds keep
...
BRAND-BOOT — the branding lift runs
...
FILTERS-1 — pack dashboard filters
...
DECLUTTER-1 + PACK-BLURB + SET-ROW-B —
...
PANEL-H batch — dashboard panels drag
...
XP-UPDATE — Quick Views' Update View
...
SETTINGS-ROAM slice 1 — branding lives
...
USERS-DURABLE 2 — the users table is
...
USERS-DURABLE + GATE-FIX-2 + ADMIN-LOCAL —
...
EXPORT-1 — exported HTML carries a data
...
VB-12 — the View Builder preview canvas
...
HOTLINK-1 + BANNER-DISMISS + BACKEND-FUTURE —
...
ACTIVITY-1 + BRAND-LINK — NEW app/activity.js
...
BRAND v2 + polish — favicon/icon-192/512/
...
WORKSPACE-LOGIN fix — direct-auth stamps the
...
HOME-EX2 + EXPLORE-LAYOUT — the 4 pack-less
...
WORKSPACE-LOGIN — the sign-in screen gains a
...
DURABLE-1 — the 8-unpinned-cap autosave
...
DESIGN-1 — Polecat design system light pass:
...
LIVE-e part 3 — the 8 hidden docs .fig-slot
...
LIVE-d slice 2 — Connections gains the
...
#118 live re-run — builder-made Views render
...
VB-5 cross-editor View opening (Kevin: "open any
...
LIVE-d slice 1 — Datasets gains the same
...
LIVE-e part 2 slice 2 (Kevin: "ALL of the docs
...
LIVE-e part 2 slice 1 (Kevin: "massive
...
LIVE-a slice 2 — the rest of the app's
...
VB-6 (Kevin's overnight View Builder queue) — the
...
VB-4 remaining major, KPI (Kevin's overnight View
...
VB-4 remaining major, Scatter (Kevin's overnight
...
VB-4 slice 2 (Kevin's overnight View Builder
...
VB-4 (Kevin's overnight View Builder queue) —
...
VB-3 (Kevin's overnight View Builder queue) —
...
LIVE-e part 1 (Kevin: "help should respond to the
...
LIVE-b (Kevin: "start with the name of the
...
VB-2 (Kevin's overnight View Builder queue) —
...
LIVE-a slice 1 (Kevin: "fix the buttons… line up
...
sync anti-flake (Kevin: "supabase seems very
...
#117 slice 5 — multi-dim series charting: the
...
VB-1 (Kevin's overnight View Builder queue) — the
...
Kevin's rail IA + sync self-heal — rail regrouped
...
#117 slice 4 — calculated columns in the View
...
#117 slice 3 — the View Builder's Filters
...
LF59 (3) — the Dashboards page toolbar
...
#117 slice 2 — the View Builder charts its
...
#117 slice 1 — the View Builder: a new "Build"
...
LF63 slice 3 — live SQL sanity hints in the
...
LF63 slice 2 — the New-data-source builder's
...
#28 (Kevin) — the marketing page gains a
...
LF65 — the legacy "Samples (115) · demo db"
...
LF57 follow-up — the Views catalog gains an
...
LF57 follow-up — Views rows/tiles show a
...
LF50 (b) — the Settings "Quick import
...
LF57 follow-up — the Views catalog gains a
...
LF57 slice 1 — a new "Views" rail section
...
Kevin live feedback — Explore's savebar reads
...
LF55(4) — the job editor's approximate output
...
LF43 slice 2 — the Studio Examples ▾ menu is
...
LF55 (5) — the job editor's step-type <select>
...
LF67 follow-up — the "New dashboard" family
...
LF51 (Explore navigator) — the dataset picker is a
...
LF51 (command center) — Repository gains a ＋ New ▾
...
bugfix (Kevin) — the builder canvas drop hint +
...
LF60 (3) — Filter/Header inspector help badges
...
LF60 slice 3 — contextual help badges deep-link into
...
LF60 slice 5 — a workspace-backend comparison table
...
LF55 (2) — job editor Filter step's value field
...
LF60 slice 2 (split) — Help splits User guides from
...
LF60 slice 2 — docs search. docs/index.html gains
...
LF39 item 2 / M7 — one-step GoTrue direct-auth at
...
LF39 (polish) — on a fresh device, a failed sign-in
...
LF60 (in-app Docs, slice 1) — the Help rail item now
...
LF40 (pack-aware tour engine) — the welcome carousel
...
LF55 (1) — job editor column dropdowns: the Filter,
...
LF55 (3) — job editor stale-preview fix: editing
...
LF34 — Style presets show which one is Active: a
...
LF66/LF59 — dashboard folder chip on the TILE: the
...
LF66 — workbooks can also go into a folder: workbook
...
LF66/LF59 — dashboard FOLDERS alongside workbooks:
...
LF63 slice 1 — the Dataset editor gains a "Browse
...
LF51 (d), last of the four workspace catalogs —
...
LF59 (2) — multi-select + bulk delete on the
...
LF59 (1) — selective export: "Export dashboards…"
...
LF56 (folder picker, slice 3) — the Browse button
...
LF54 (slice 2) — density tightening: the two
...
LF56 (folder picker, slice 2) — the shared
...
LF56 (folder picker, slice 1) — a shared
...
LF52 (analysis→View, app-UI slice) — user-facing
...
LF64(3) a11y — the dataset builder's "Date token"
...
LF69(c) — the Interactive GL choropleth renderer
...
LF45 (Save-as half) — the builder toolbar's "Save as"
...
LF32(a) — the dataset builder's offline Query Preview
...
LF64 slice 3 — a "Date token" button on the dataset
...
LF64 slice 2 — the dynamic date-token set grows to
...
LF64 slice 1 — built-in DYNAMIC date tokens usable
...
LF66 slice 3 (library reorg, part 6 cont.) — a
...
LF48 (LOCKED BUILD ORDER step 4, the last
...
UX sweep 2026-07-28 — KPI tiles without a
...
LF66 slice 2 (library reorg, part 6) — dragging a
...
LF52 (extend) — finish the widget/analysis→View
...
LF66 slice 1 (library reorg) — the Studio data
...
LF46 (⋯ teardown, slice 2) — Tour, Theme,
...
LF51 slice 4 (nav IA spec (b), "right-aligned
...
LF51 slice 3 (nav IA spec (d)) — the list ⇆ tile
...
LF51 slice 2 (nav IA spec (d), "list + rich-tile
...
LF51 spec (a) — row NAMES no longer
...
LF49 slice 3 upgrade — the PowerPoint (.pptx)
...
LF51 slice 1 (nav IA spec (c), "show a full
...
LF49 slice 3 — Export ▾ → "PowerPoint (.pptx)",
...
LF50 — removed the stray in-builder Low/High
...
LF49 slice 2 — Export ▾ → "Word document (.docx)":
...
LF54 slice 1 — left-align workspace content
...
LF49 slice 1 — Export ▾ → "Excel workbook (.xlsx)":
...
LF52 — the dashboard item "widget" is renamed to
...
LF48 slice 1 — a shared .mode-exit "exit pill"
...
LF45 — the "Open a dashboard" picker was a bare text
...
LF46 (⋯ teardown) — "Demo mode" is no longer a
...
LF53 — de-jargoned the user-facing "CDF"/"Dashboard
...
LF62 slice 1 — a ✨ sparkle button on the
...
LF69(d) — the per-panel PNG/CSV/standalone-HTML
...
LF69(a) — the panel action row now moves the
...
the read-only Viewer bar (#viewerBar) is now
...
the DA inspector's Cache section (Enabled + Duration)
...
the Studio widget + KPI editor's data-source field
now reads "Dataset" instead of "Query (data access)" (#119, Kevin live QA). app/studio.js
changed, so precached copies roll.

## v279

UX sweep 2026-07-28 #1 — added a phone-reachable
...
the offline sample generator now emits ONE row per
...
LF70 — Home's "Browse examples" card now jumps
...
viewer mode (#106/#107) now renders SAMPLE-backed
...
LF67 — a Quick-import build now warns before
...
the app Color-theme picker reaches parity with the
...
LF68 — fixed a "sensitive_pct"-style KPI column
...
the "Restore unsaved work" banner is now opt-in — a new
...
LF40 slice 1 — the welcome tour opens with an
...
Settings copy cleanup — a one-line Color-theme blurb, a
...
Home's "Welcome back" greeting now personalizes to the
...
sign-in polish — the password field gets the eye/eye-off
...
LF41 slice 2 — Add/Edit user gains a "Copy my current
...
panel zoom fills the window (fill-CSS injected into the
...
Standard fleet topbar — Slice C: Studio's
...
connection self-heals on boot — initSync now
...
Standard fleet topbar — Slice B: Studio's
...
global standard topbar (Slice A) — #topbar is
...
LF42 slice 3 — Settings → Workspace backend's
...
LF42 slice 2 — Add/Edit user gains an "Assigned
...
durable Supabase reads — a read that hits a
...
first-admin bootstrap — Go live now seeds the
...
LF42 slice 1 — Admin "Backends" card lets an
...
LF39 — cross-device sign-in no longer gives a
...
LF43 — installing a sample pack now
...
Quick import now defaults to High creativity
...
LF44 — Home's "New dashboard"/"Quick
...
LF38 — app-wide password reveal toggle
(app/studio.js, app/connections.js, app/icons.js, app/studio.css all changed), so
precached copies need to roll.

## v250

Track L sweep (orphaned-key lens, round 6) — the PDF
...
modal outside-click dismiss now requires the
...
N-DATA follow-up — the Checks section's collapsed
...
Google Sheets FOLLOW-UP — private-sheet OAuth
...
Track H sweep (code-health lens) — the Viewer
...
Track L sweep (performance-budget lens) —
...
N-DESIGN follow-up — a third Card style,
...
Track H sweep — myDACard's Duplicate/Delete
...
Track L sweep (orphaned-key lens, round 5) —
...
N-DIST follow-up — a diff-based "Share just my
...
Track H sweep — row/card title buttons across
...
Track L sweep (chart-extension API lens) — every
...
N-DATA follow-up — KPI period-over-period gets
...
N-DATA innovation sweep — KPI period-over-
...
LF24 slice 3 — the creativity dial +
...
LF24 slice 2 — the auto-build engine.
...
LF24 slice 1 — Quick import. A new
...
Post-overhaul backlog item 3, OTHER
...
changed, so the precached copies need to roll.

## v228

REGRESSION FIX — the v227 PostgREST
...
so the precached copy needs to roll.
...
post-overhaul backlog item 7's
...
M7 slice 7 (Conservation Insight
...
UX5 slice 4 (the 4px-base spacing
...
UX5 slice 2 (quality track) — the 7
...
R5+ slice 9 (studio.js module extraction,
...
R5+ slice 8 (studio.js module extraction,
...
UX5 slice 1 (quality track) — a named
...
R5+ slice 7 (studio.js module extraction,
...
R5+ slice 6 (studio.js module extraction,
...
LF18(b) — a fifth guided tour, "Connections &
...
LF18(b) — a fourth guided tour, "Prep data (Jobs)",
...
LF18(d) — Home's Examples section hint now names
...
LF18(a) — Home's quick-action cards reworded to
...
R5+ slice 5, part 2 (tech-debt track) — the
...
LF22 slice 4 — the counties→custom-region MAPPING
...
LF22 slice 3 — a new 5-digit ZIP (ZCTA) choropleth
...
LF22 slice 2 — a new Congressional districts
...
LF22 slice 1 — HUC8 watersheds now cover the
...
LF19 slice 10 — every Inspector section's
...
LF19 slice 9 — Advanced-mode inspector
...
LF13(d) slice 3 — the job editor's
...
LF13(d) slice 2 — the job editor gains a
...
LF13(d) slice 1 — the job editor gains a
...
LF19 Inspector slice 8 — the Data Source
...
LF16/LF2(c) — the generic showcase gallery (8
...
LF19 Inspector slice 7 — three more Inspector
...
LF19 Inspector slice 6 — the Widget/Panel
...
LF19 Inspector slice 1 — the right Inspector
...
LF19 next slice — the Data panel's search
...
LF19 next slice — the "+ New" shortcut on
...
LF19 slice 1 — the Studio library's "This
...
LF34 — the Settings "Style presets" name field +
...
LF25(b) — the Inspector's generic select-type chart
...
LF25(a) — every widget can export itself as a
...
LF30 — a marketing chart-type gallery on the
...
LF21 — the dashboard title/header banner becomes
...
LF23 slice 2 — role gating + Edit-in-Studio +
...
LF23 slice 1 — Viewer mode. A new standalone
...
UX6 icon migration — LAST slice. The chart-
...
UX6 icon migration — the first-run Welcome
...
M7 slice 6 — in-app account provisioning.
...
LF16 — the Settings Account card's own
...
UX6 (icon migration, carets slice) — the
...
LF13(b) — a second (or third) "aggregate" job
...
UX6 (currentColor icon migration, slice 4) —
...
LF13(c) — the job editor gains a "uniqueKey" step
...
LF13(a) — the job editor's group-by/metric/
...
UX6 (currentColor icon migration, slice 3) —
...
UX6 (currentColor icon migration, slice 2) —
...
LF20 — the builder toolbar's Open/Save as/Close
...
LF36 slice 2 — the PDF export's deferred "sizing/
...
LF35 slice 2 — the movable half of the choropleth
...
LF36 slice 1 — a new "PDF (print)" entry in the
...
R5+ slice 5 (tech-debt track) — the version-history
...
LF37 — Home Examples "+N more" footer is now a real
...
QA-04 slice 2 — duplicate-named dashboards/analyses/
...
QA-06 — the restore-unsaved-work banner now reserves
...
QA-05 — icon-only action buttons (Explore's saved-
...
QA-04 — brand-new blank/duplicated dashboards no
...
QA-03 — Explore's featured county demo now opens
...
QA-02 — credential-storage copy is now state-aware
...
QA-01 — connection credential inputs get a stable
...
Branding is now an app-wide ADMIN card
...
LF2 — an 8th Conservation example, "The Story So
...
LF2 — a 7th Conservation example, "Year-over-
...
R5+ slice 4 (tech-debt track) — the N-FUN
...
LF9 slice 3 — Back also closes Explore's dataset/
...
LF9 slice 2 — Back now closes an open overlay
...
LF9 slice 1 — Back/Forward now walks section
...
LF2 — a 6th Conservation example, "County-Level
...
LF2 — a 5th Conservation example, "Provider
...
LF2 — a 4th Conservation example, "Program
...
M7 slice 3 — the data migration RLS needs before
...
M7 slice 2 — optional Supabase Auth (GoTrue)
...
R5+ slice 3 (tech-debt module-extraction track) —
...
R5+ slice 2 (tech-debt module-extraction track) —
...
LF4(c) — the SVG choropleth renderer's "State
...
LF20 — the dashboard's own light/dark render mode
...
LF2 — a 3rd Conservation example, "Watershed-Scale
...
LF26 — "Save as…" + overwrite protection: a
...
LF6 slice 2 — folded the choropleth/ensemble-
...
LF6 — per-panel download chrome (image + data),
...
UX7 (quality track) — mobile 44px touch targets
...
LF7 — the featured Conservation Insight dashboard
...
LF28 — the GL choropleth's interactive pan/zoom
...
LF27(b) — Studio gains a Close button
...
LF27(a) — a fresh boot (no saved section)
...
LF32 — a job-output rollup's "statecode"
...
LF33 — the left rail's brand mark now
...
ensemble chart contrast fix — the
...
Datasets list row fix — a heavily
...
Explore's own dataset picker, its saved-analysis
...
"Clear local data" now also signs you out
...
R3 — the stylePresets/templateVarSets/
...
R3 slice 1 — the 8 default-style config
...
UX9 — bare .btn buttons inside modals (e.g.
...
LF29a typography polish — object names/ids
...
M5 NEXT — "drag-to-file", the last of
...
M5 NEXT — Repository gains a "+ New
...
M5 NEXT — dashboards gain the same
...
LF11 — Explore's ambiguous single
...
M5 folder pilot slice 4 — Analyses
...
M5 folder pilot slice 3 — Jobs gain
...
M5 slice 2 — Repository's rows are
...
M5 slice 1 — new Repository section
...
LF5(b) — color-token pickers gain a
...
polecat-shell v0.5.4 — brand-color
app-switcher tiles (vendored catalog + shell + site-chrome changed).

## v88

FIX — clicking a panel's dataset
...
faint polecat.live suite link in the rail
(app/index.html + app/studio.css touched).

## v86

LF2 — 2 new Conservation-themed example
...
Calmer Data panel — the compound
...
FIX — updatedAt is now typed BIGINT in
...
M4.2 per-section rights (the second
...
FIX — an admin who just signed in
...
M4.2 slice 5 (last object type) — the
...
M4.2 slice 4 — the same private/public
...
M4.2 slice 3 — the same private/public
...
M4.2 slice 2 — the same private/public
...
M4.2 slice 1 — a private/public toggle
...
M4 admin — a rail area (admins only)
...
the dashboard header text objects are
...
choropleth hover no longer sticks —
...
the Conservation Insight featured
...
new "Conservation" dashboard theme,
...
Explore polish — clicking a saved
...
M3.1 — sign-in replaces the passcode.
...
fleet-standard topbar, step 1 — the
...
FIX — the Explore rollup was applied
...
FIX — faint secondary buttons on the
...
FIX — choropleth hover highlights
...
canvas consistency + header-off. The
...
FIX — primary call-to-action buttons
...
Conservation Insight quick wins —
...
N-DESIGN theme studio follow-up —
...
N-DESIGN theme studio first cut —
...
Track L sweep (orphaned-key lens,
...
post-overhaul item 4 continued —
...
pin/favorite toggle on Datasets and
...
fixed the dark/light theme toggle not reaching
Home/Dashboards/Datasets/Connections/Jobs/Settings (app/studio.css changed).

## v25

Track L sweep — removed a dead orphaned function
from app/studio.js (content changed, so precached copies need to roll).

## v24

V9 scientific-honesty polish slice 4 — "Last updated"
