...
<a id="groomed-2026-08-10"></a>

## ✓ Shipped out of the ▶ NOW queue (grooming pass 4, 2026-08-10)

> Struck entries moved verbatim from STATUS.md ▶ NOW per docs/BACKLOG.md grooming — 14 entries,
> 490 lines, nothing deleted, nothing renumbered, NOW's order untouched. Both triggers had fired:
> the queue held 14 carcasses (the rule's threshold is 5) and, once they were read past, only two
> unmarked items — both of which turned out to be waiting on Kevin, which is the second half of
> this pass.
>
> **The 24 hours this pass drains** were the lane's densest yet: SP-6 shipped end to end (three
> slices, v963–v965), and N32–N37 plus N40/N42/N43 closed the View Builder and catalog defects
> Kevin filed on 2026-08-09.
>
> **Two entries here share one ID and that is not a mint error.** `N36` appears twice — the
> struck slice record and, further down, the ORIGINAL spec text it was separated from at some
> earlier edit. Grooming pass 3 already ruled on it ("one ID, one item"); both halves move
> together so the pair stays legible.
>
> **One finding leaves the queue with its item and is deliberately NOT lost:** SP-6's body
> recorded that the choropleth's colour classes are LINEAR only, so any power-law measure paints
> one colour (407 of 436 congressional districts landed in the lowest sixth of the range). It is
> a chart-capability slice of its own and not the loop's to rank, so it stays recorded as a note
> at the foot of ▶ NOW rather than being promoted. The measurement lives in the SP-6 text below.
>
> **Pass 2 (PR #623) is still open on `hold`**, unanswered since 2026-08-07. As at pass 3, that
> is why this pass proposes no batch of its own — a third unanswered proposal would be noise.
