...
// AUD-08: the boot-path companion. js/changelog.js is ~680KB of history that only the
// "What's new" feed ever reads, yet the app used to load (and precache) all of it just to
// print a version number in the footer. This tiny sibling carries the two facts boot needs
// — the latest version + the newest entry — and is regenerated from the same source of
// truth on every normalize run, so the pair can never drift.
const HEAD_FILE = path.resolve(__dirname, "..", "js", "changelog-head.js");
...
// ── the boot-path head file (AUD-08) ──
// A classic script (not a module) so it runs during parse, before app/fleet.js reads
// STUDIO_LATEST_VERSION for the unseen dot. Deliberately carries the newest entry WITHOUT
// its `items` — the footer needs v/title/ts, and leaving the bullet list out keeps this
// file in the hundreds-of-bytes range no matter how long a release note gets.
const latestEntry = canon.reduce((a, b) => (b.v > a.v ? b : a), canon[0]);
const headText =
...
window.STUDIO_LATEST_VERSION = ${latestEntry.v};
window.STUDIO_LATEST = ${JSON.stringify({ v: latestEntry.v, title: latestEntry.title, kind: latestEntry.kind || "", ts: latestEntry.ts })};
...
fs.writeFileSync(HEAD_FILE, headText);
console.log(`changelog-normalize: ${canon.length} entries, top v${canon[0].v}; stamped ${stamped} empty ts with ${NOW}; manager-parse OK.`);
console.log(`changelog-normalize: js/changelog-head.js regenerated (v${latestEntry.v}, ${headText.length} bytes).`);
