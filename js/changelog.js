...
export const CHANGELOG = [
  ...
  },
  {
    ...
    kind: 'fix',
    ts: '2026-08-09T07:00:44.000Z',
    items: [
      'Every picture in the carousel at the top of analytics.polecat.live is a genuine capture of the running app, regenerated by a script rather than drawn by hand. They had not been regenerated since 31 July, and the app has moved a long way since. They have all been retaken — and taking them turned up three things that were wrong, two of them for longer than that.',
      'The slide captioned "The Dashboard Builder in dark theme with the data and inspector panels" was a picture of the Home screen. Loading a dashboard into the builder does not switch you to the builder, so the capture never left the page it started on, and a congratulatory pop-up ("All clear — this dashboard has zero warnings") was sitting in the middle of it. That slide now shows the Dashboard Builder, with the data panel on the left and the inspector on the right, exactly as the caption says.',
      'The watershed map slide contradicted its own caption. Last week the caption was corrected to say that HUC8 watersheds are one of the six map scales the app ships, rather than boundaries you have to supply yourself — but the correction could not reach the words printed inside the picture, which still read "A custom geography". Anyone reading the slide got both claims at once. The picture now says what the caption says.',
      'The Dashboards slide had quietly become a list. The caption promises "every saved dashboard as searchable tiles", and a recent change made the list the opening view for every catalog, so the screenshot silently followed it. The shot now asks for the tile grid explicitly, so a future change to the default cannot make the picture disagree with the words beside it again.',
      'The dev gate now measures the copy that is printed inside a screenshot the same way it already measures the copy printed beside it: the number of map scales a picture claims, and the rule that a scale the app ships is never described as one you must bring, are both read off the map chart\'s own menu. A picture cannot go stale in a direction the page has already been corrected in.',
    ],
  },
  {
    v: 917,
